...
public class CopyOfAVL {
	...
	public void Insertar(int llave, String dato){
		...
		if (raiz==null) {
			...
			NodoAVL abueloAVL=null;
			NodoAVL padreAVL=null;
			NodoAVL auxAVL=raiz;
			while (auxAVL!=null) {
				nuevo.padre=auxAVL;
				if (nuevo.llave<=auxAVL.llave) {
					abueloAVL=padreAVL.padre;
					padreAVL=auxAVL.padre;
					auxAVL=auxAVL.hijoIzquierdo;
					
				} else {
					abueloAVL=padreAVL.padre;
					padreAVL=auxAVL.padre;
					auxAVL=auxAVL.hijoDerecho;
				}
			}
			rotarAVL(nuevo, padreAVL, abueloAVL);
		}
		largo++;
		repeticiones+=llave;
	 ...
	 * @return
	 */
	private NodoAVL rotarAVL (NodoAVL nuevo, NodoAVL padreAVL,NodoAVL abueloAVL){
		NodoAVL aux=null;
		if (Math.abs(obtenerFactorEquilibrio(padreAVL.hijoIzquierdo)-obtenerFactorEquilibrio(padreAVL.hijoDerecho))==2){
			if (abueloAVL.hijoIzquierdo.hijoIzquierdo==nuevo) {
				aux=rotacionSimpleDerecha(nuevo);
			}
			else if (abueloAVL.hijoDerecho.hijoDerecho==nuevo) {
				aux=rotacionSimpleIzquierda(nuevo);
			}
			else if (abueloAVL.hijoIzquierdo.hijoDerecho==nuevo) {
				aux=rotacionDobleDerecha(nuevo);
			} 
			else {
				aux=rotacionDobleIzquierda(nuevo);
			}
		}
		else {
			aux=padreAVL;
